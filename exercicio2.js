...
class Gallery {
  ...
  static servidorDasImagens =
    "https://fegemo.github.io/cefet-web/images/osiris";
  ...
  static element = document.getElementById("slide")


  static handleSlide(input) {
    switch (input) {
      case "+":
        this.index = this.index === this.imagens.length - 1 ? 0 : this.index + 1;
        break
      case "-":
        this.index =
          this.index === 0 ? this.imagens.length - 1 : this.index - 1;
        break
    }

    this.element.src = `http://fegemo.github.io/cefet-web/images/osiris/${this.imagens[this.index].arquivo}`
    this.element.alt = this.imagens[this.index].descricao
